class GoogleCivicsApi
  ...
  def self.get_officials(user)
    ...
    self.parse_offices(parsed_data["offices"], parsed_data["officials"])
  end 
  ...
  def self.parse_offices(offices_array, officials_array) #pass in parsed_data["offices"]
    offices_array.each do |office|
      office["officialIndices"].each do |index|
        self.create_new_official_instance(index, office, officials_array) #index from official indices
        #office hash
  ...
  def self.create_new_official_instance(index, office, officials_object_array)
    ...
    new_politician = Politician.find_or_create_by(
      ...
      photo_url: politician_object["photoUrl"] ? politician_object["photoUrl"] : "Unknown",
